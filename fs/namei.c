...
static inline int may_follow_link(struct nameidata *nd)
{
	...
	if (nd->inode && susfs_need_to_spoof_sus_path(nd->inode, from_kuid(current_user_ns(),nd->inode->i_uid)))
	{	
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (inode && susfs_need_to_spoof_sus_path(inode, from_kuid(current_user_ns(),inode->i_uid)))
	{
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!IS_ERR(dentry) && dentry->d_inode && susfs_need_to_spoof_sus_path(dentry->d_inode, from_kuid(current_user_ns(),dentry->d_inode->i_uid)))
	{
		dput(dentry);
		return ERR_PTR(-ENOENT);
	}
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	inode = path->dentry->d_inode;
	if (!retval && inode &&	susfs_need_to_spoof_sus_path(inode, from_kuid(current_user_ns(),inode->i_uid)))
	{
		putname(name);
		return -ENOENT;
	}
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (susfs_need_to_spoof_sus_path(inode, from_kuid(current_user_ns(),inode->i_uid))) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode && susfs_need_to_spoof_sus_path(child->d_inode, from_kuid(current_user_ns(),child->d_inode->i_uid)))
	{	
		error = inode_permission2(mnt, dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (susfs_need_to_spoof_sus_path(inode, from_kuid(current_user_ns(),dentry->d_inode->i_uid))) {
		return -ENOENT;
	}
#endif
...
static int may_o_create(const struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (dentry->d_inode && susfs_need_to_spoof_sus_path(dentry->d_inode, from_kuid(current_user_ns(),dentry->d_inode->i_uid)))
	{		
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (susfs_need_to_spoof_sus_path(dentry->d_inode, from_kuid(current_user_ns(),dentry->d_inode->i_uid))) {
			dput(dentry);
			return -ENOENT;
		}
#endif
